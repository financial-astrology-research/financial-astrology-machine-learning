...
#             4) Validate fit using Actbin Z daily price change (buy / sell) instead of Effect
#             5) GA feature detection that fit to maximize Rsquared train data.
#             6) Fit 5 weak learners for Z diff percent change.
...
searchModel <- function(symbol) {
  ...
  prepareModelData <- function(params) {
    ...
    aspectView <- merge(
      securityData[, c('Date', 'zdiffPercent', 'Actbin', 'Eff')],
      dailyAspects, by = "Date"
    )

    trainIndex <- createDataPartition(aspectView$zdiffPercent, p = 0.80, list = FALSE)
    aspectViewTrain <- aspectView[trainIndex,]
    aspectViewValidate <- aspectView[-trainIndex,]
    useFeatures <- names(dailyAspects)[-1]
    selectCols <- c('zdiffPercent', useFeatures)
  ...
  findRelevantFeatures <- function(solution) {
    ...
    for (i in 1:5) {
      ...
      validateMAE <- mae(modelData$validate$zdiffPercent, validateDiffPercentPred)
      validateRMSE <- rmse(modelData$validate$zdiffPercent, validateDiffPercentPred)
      validateR2 <- cor(modelData$validate$zdiffPercent, validateDiffPercentPred)^2
