...
#            12) GA solution aspect energy search within 0-5 interval.
...
searchModel <- function(symbol) {
  ...
  parseSolutionParameters <- function(solution) {
    ...
    pySelect <- pySelectAll[solution[1:11] == 1]
    aspectSelect <- aspectSelectAll
    aspectsEnergy <- solution[12:gaParamsNum]
  ...
  gar <- ga(
    ...
    upper = c(rep(1, 13), rep(5, gaParamsNum-13)),
