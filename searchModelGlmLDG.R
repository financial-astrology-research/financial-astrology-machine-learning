...
#             4) Use kknn in favor of glm as weak learners algorithm.
#             5) Change data split to 80/20 proportion.
#             6) Include SU in fast planet.
#             7) Optimize weak learners and ensamble for Kappa.
#             8) Train weak learners with different aspects features combinations.
#             9) Fit weak learners for MA trend and ensamble for Actbin to generalize for daily change.
#            10) Use different cycles for weak learners.
...
useFeatures3 <- allFeatures[grep('^ME|^VE', allFeatures)]
fitModel3 <- modelTrain(
  "glm", useFeatures3, 2, 4, "3"
)

useFeatures4 <- allFeatures[grep('^MO|^SU', allFeatures)]
fitModel4 <- modelTrain(
  "glm", useFeatures4, 2, 4, "4"
)

useFeatures5 <- allFeatures[grep('^MO|^MA', allFeatures)]
fitModel5 <- modelTrain(
  "glm", useFeatures5, 2, 4, "5"
)
...
fwrite(dailyAspects[, ..exportCols], paste("./predictions/", symbol, "-predict-glmLDG-ensamble", ".csv", sep = ""))
