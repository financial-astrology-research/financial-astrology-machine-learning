...
#            12) Increase default aspects orb limit to 6.
...
#            14) Minor aspects set: 45/135/150/180, Medium: 30, Major: 0/60/90/120
...
modelId <- "ensamble-gakknn-VACGBFCB"
zscoreCut <- 2
maPriceFsPeriod <- 2
maPriceSlPeriod <- 3
...
dailyAspectsRows[aspect %in% c(45, 135, 150, 180) & orb > 2, filter := T]
dailyAspectsRows[aspect %in% c(30) & orb > 3, filter := T]
dailyAspectsRows[aspect %in% c(0, 60, 90, 120) & orb > 4, filter := T]
