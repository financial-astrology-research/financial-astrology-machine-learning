...
#             1) Planets MO, ME, VE, SU, MA fast planets applying to all slow planets, asteroids and NN.
...
#            12) KKNN K set to 8.
#            13) Include pxSelect in GA solution search.
...
searchModel <- function(symbol) {
  ...
  parseSolutionParameters <- function(solution) {
    pxSelect <- pxSelectAll[solution[1:5] == 1]
    pySelect <- pySelectAll[solution[6:nBits] == 1]
