# Title     : Daily generalized aspects energy with planets px * py weight and speed decay data KNN regression model
...
#             1) Planets MO, ME, VE, SU, MA, CE, VS, JU, SA, NN, CH, UR, NE fast planets applying to all planets, asteroids and NN.
...
searchModel <- function(symbol) {
  ...
  prepareDailyAspects <- function(pxSelect, pySelect, planetWeight, orbLimit) {
    dailyAspects <- dailyAspectsGeneralizedEnergySum(
      ...
      energyFunction = partial(dailyAspectsAddEnergyWeightWithDecay4, speedDecay = 0.59, planetWeight = planetWeight)
    )
  ...
  prepareModelData <- function(params) {
    c(pxSelect, pySelect, planetWeight, orbLimit) %<-% params
    dailyAspects <- prepareDailyAspects(pxSelect, pySelect, planetWeight, orbLimit)
  ...
  modelTrain <- function(params) {
    c(pxSelect, pySelect, planetWeight, orbLimit) %<-% params
    cat("Using PX:", pxSelect, "- PW:", planetWeight, "- ORB:", orbLimit, "\n")
