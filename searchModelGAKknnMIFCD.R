# Title     : Daily generalized aspects count + planetY aspects energy activation data KNN regression model
...
#             1) Planets MO, ME, VE, SU, MA planets applying to all slow planets, asteroids and NN.
...
#            14) Minor aspects set: 120/135/150/180, Medium: 30/45/60 Major: 0/90
...
dailyAspectsRows[aspect %in% c(0, 90) & orb > 5, filter := T]
...
searchModel <- function(symbol) {
  ...
  prepareDailyAspects <- function(pxSelect, pySelect, aspectSelect, aspectsEnergy) {
    ...
    dailyAspects <- merge(dailyAspectsGeneralizedCount, dailyPlanetYActivationEnergy, by = "Date")
