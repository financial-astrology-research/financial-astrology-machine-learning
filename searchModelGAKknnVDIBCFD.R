...
#             1) Planets MO, ME, VE, SU, MA, JU, SA planets applying to all planets, asteroids and NN.
...
#            12) Classic aspects set 10 were all aspects orbs are calculated with 6 degrees orb
#                and filtered to 4 (major), 3 (medium), 2 (minor) for this model.
#            13) Minor aspects set: 135/150, Medium: 30/45/60/180 Major: 0/90/120
#            14) Add MO to pxSelect filtering only 0, 60, 90, 120, 180 aspects.
...
modelId <- "ensamble-gakknn-VDIBCFD"
zscoreCut <- 2
maPriceFsPeriod <- 2
maPriceSlPeriod <- 3
...
dailyAspectsRows[aspect %in% c(135, 150) & orb > 2, filter := T]
dailyAspectsRows[aspect %in% c(30, 45, 60, 180) & orb > 3, filter := T]
dailyAspectsRows[aspect %in% c(0, 90, 120) & orb > 4, filter := T]
dailyAspectsRows[aspect %ni% c(0, 60, 90, 120, 180) & p.x == 'MO', filter := T]
...
searchModel <- function(symbol) {
  ...
  prepareDailyAspects <- function(pxSelect, pySelect, planetWeight, aspectWeight) {
  ...
  prepareModelData <- function(params) {
    c(pxSelect, pySelect, planetWeight, aspectWeight) %<-% params
    dailyAspects <- prepareDailyAspects(pxSelect, pySelect, planetWeight, aspectWeight)
  ...
  modelTrain <- function(params) {
    c(pxSelect, pySelect, planetWeight, aspectWeight) %<-% params
    cat("Using PX:", pxSelect, "- PW:", planetWeight, "- AW:", aspectWeight, "\n")
  ...
  parseSolutionParameters <- function(solution) {
    ...
    planetWeight <- solution[1:14] / 10
    aspectWeight <- solution[15:gaParamsNum] / 10
