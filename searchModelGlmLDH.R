# Title     : Daily planet X emitted aspects count.
...
#             2) CV folds to 5 with 10 repeats.
#             3) Validate fit using Actbin daily price change (buy / sell) instead of Effect
#             4) Fit weak learners based on MA(2, 3) effect to smooth price variations.
#             5) Fit ensamble for Actbin to generalize for daily change.
#             6) Data split to 80/20 proportion.
#             7) Optimize weak learners and ensamble for Kappa.
#             8) Orb set to 4.

# CONCLUSION:
# The purpose of this model was to try to determine the aspects polarity
# using coficients plots but with the emitter aspects representation the model cannot fit
# to reasonable level of accuracy. Skipped to fit for predictions.
...
fitModel1 <- modelTrain(
  "glm", allFeatures, maPriceFsPeriod, maPriceSlPeriod, "1"
)

fitModel2 <- modelTrain(
  "glm", allFeatures,maPriceFsPeriod, maPriceSlPeriod, "2"
)

fitModel3 <- modelTrain(
  "glm", allFeatures, maPriceFsPeriod, maPriceSlPeriod, "3"
)

fitModel4 <- modelTrain(
  "glm", allFeatures, maPriceFsPeriod, maPriceSlPeriod, "4"
)

fitModel5 <- modelTrain(
  "glm", allFeatures, maPriceFsPeriod, maPriceSlPeriod, "5"
)

fitModel1 %>% print()
fitModel1 %>% varImp()
fitModel1$finalModel %>% coefplot()

fitModel2 %>% print()
fitModel2 %>% varImp()
fitModel2$finalModel %>% coefplot()

fitModel3 %>% print()
fitModel3 %>% varImp()
fitModel3$finalModel %>% coefplot()

fitModel4 %>% print()
fitModel4 %>% varImp()
fitModel4$finalModel %>% coefplot()

fitModel5 %>% print()
fitModel5 %>% varImp()
fitModel5$finalModel %>% coefplot()
...
fwrite(dailyAspects[, ..exportCols], paste("./predictions/", symbol, "-predict-glmLDH-ensamble", ".csv", sep = ""))
