...
#             diffsqHxL price change estimation.
# Purpose   : Predict daily traiding signal action category (buy / sell) from estimated daily price change.
#             1) Planets MO, ME, VE, SU fast planets applying to all planets and VS, CH.
...
#             6) Fit 5 weak learners for diffsqHxL change.
...
#            11) Perform diffsqHxL zscore outliers drop.
...
searchModel <- function(symbol) {
  ...
  securityData <- securityData[abs(zdiffsqHxL) <= zscoreCut]
  hist(securityData$diffsqHxL)
  ...
  prepareModelData <- function(params) {
    ...
    aspectView <- merge(
      securityData[, c('Date', 'diffsqHxL', 'HxLEff')],
      dailyAspects, by = "Date"
    )

    trainIndex <- createDataPartition(aspectView$diffsqHxL, p = 0.80, list = FALSE)
    aspectViewTrain <- aspectView[trainIndex,]
    aspectViewValidate <- aspectView[-trainIndex,]
    useFeatures <- names(dailyAspects)[-1]
    selectCols <- c('diffsqHxL', useFeatures)
  ...
  findRelevantFeatures <- function(solution) {
    ...
    for (i in 1:5) {
      ...
      validateMAE <- mae(modelData$validate$diffsqHxL, validateDiffPercentPred)
      validateRMSE <- rmse(modelData$validate$diffsqHxL, validateDiffPercentPred)
      validateR2 <- cor(modelData$validate$diffsqHxL, validateDiffPercentPred)^2
