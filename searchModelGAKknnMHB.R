# Title     : Daily generalized aspects count + comb planets aspects energy data KNN regression model
...
#            11) KKNN K param set to 6.
#            12) Aspect energy function without orb decay.
...
searchModel <- function(symbol) {
  ...
  prepareDailyAspects <- function(pxSelect, pySelect, aspectSelect) {
    ...
    dailyAspectsPlanetCombEnergy <- dailyAspectsPlanetCombGeneralizedEnergy(
      dailyAspects = dailyAspectsRows,
      orbLimit = orbLimit,
      pxSelect = pxSelect,
      pySelect = pySelect,
    ...
    if (is.null(dailyAspectsPlanetCombEnergy)) {
      return(NULL)
    }

    dailyAspects <- merge(dailyAspectsGeneralizedCount, dailyAspectsPlanetCombEnergy, by = "Date")
