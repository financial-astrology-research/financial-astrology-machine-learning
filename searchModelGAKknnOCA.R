...
#             6) Fit 5 weak learners for difflogHLMASxF change.
...
#            11) Perform difflogHLMASxF zscore outliers drop.
...
searchModel <- function(symbol) {
  ...
  securityData <- securityData[abs(zdifflogHLMASxF) <= zscoreCut]
  hist(securityData$difflogHLMASxF)
  ...
  prepareModelData <- function(params) {
    ...
    aspectView <- merge(
      securityData[, c('Date', 'difflogHLMASxF', 'HxLEff')],
      dailyAspects, by = "Date"
    )

    trainIndex <- createDataPartition(aspectView$difflogHLMASxF, p = 0.90, list = FALSE)
    aspectViewTrain <- aspectView[trainIndex,]
    aspectViewValidate <- aspectView[-trainIndex,]
    useFeatures <- names(dailyAspects)[-1]
    selectCols <- c('difflogHLMASxF', useFeatures)
  ...
  findRelevantFeatures <- function(solution) {
    ...
    for (i in 1:5) {
      ...
      validateMAE <- mae(modelData$validate$difflogHLMASxF, validateDiffPercentPred)
      validateRMSE <- rmse(modelData$validate$difflogHLMASxF, validateDiffPercentPred)
      validateR2 <- cor(modelData$validate$difflogHLMASxF, validateDiffPercentPred)^2
  ...
  trainIndex <- createDataPartition(aspectView$HxLEff, p = 0.90, list = FALSE)
  aspectViewTrain <- aspectView[trainIndex,]
  aspectViewValidate <- aspectView[-trainIndex,]
