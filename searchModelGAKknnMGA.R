# Title     : Daily generalized aspects + planetY activation mean orb data KNN regression model
...
searchModel <- function(symbol) {
  ...
  prepareDailyAspects <- function(pxSelect, pySelect, aspectSelect) {
    ...
    dailyPlanetYAspectMeanOrb <- dailyPlanetYAspectMeanOrb(
      dailyAspects = dailyAspectsRows,
      orbLimit = orbLimit,
      pxSelect = pxSelect,
      pySelect = pySelect,
    ...
    if (is.null(dailyPlanetYAspectMeanOrb)) {
      return(NULL)
    }

    dailyAspects <- merge(dailyAspectsGeneralizedCount, dailyPlanetYAspectMeanOrb, by = "Date")
