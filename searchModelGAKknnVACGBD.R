...
#            12) Increase default aspects orb limit to 3.
#            13) New aspects set were all aspects orbs are calculated with 6 degrees orb
#                and filtered to 3 (major), 2 (medium), 1 (minor) for this model.
#            14) Minor aspects set: 135, Medium: 45/150, Major: 0/30/60/90/120/180
...
modelId <- "ensamble-gakknn-VACGBD"
zscoreCut <- 2
maPriceFsPeriod <- 2
maPriceSlPeriod <- 3
...
dailyAspectsRows[aspect %in% c(135) & orb > 1, filter := T]
dailyAspectsRows[aspect %in% c(45, 150) & orb > 2, filter := T]
dailyAspectsRows[aspect %in% c(0, 30, 60, 90, 120, 180) & orb > 3, filter := T]
