# Title     : Daily natal chart and mundane generalized aspects and planets activation count KKNN ensamble model.
...
#             11) Use natal chart and mundane aspects.
...
searchModel <- function(symbol) {
  symbolParts <- unlist(strsplit(symbol, "-"))
  transitsPathFile <- paste0("./dplanets/", symbolParts[1], "_natal_transits.csv")
  cat("Using transits:", transitsPathFile, "\n")
  dailyTransits <- fread(transitsPathFile)
  dailyTransits <- dailyTransits[minOrb <= orbLimit,]
  dailyTransits[, aspect := str_replace(aspect, 'a', '')]
  dailyTransits[, Date := as.Date(Date)]
  setnames(dailyTransits, "pX", "p.x")
  setnames(dailyTransits, "pY", "p.y")

  dailyTransitsCount <- dailyAspectsGeneralizedCount(
    dailyAspects = dailyTransits,
    orbLimit = orbLimit,
    pxSelect = pxSelect,
    pySelect = pySelect,
  ...
  dailyTransitsPlanetYCount <- dailyPlanetYActivationCount(
    dailyAspects = dailyTransits,
    orbLimit = orbLimit,
    pxSelect = pxSelect,
    pySelect = pySelect,
  ...
  dailyTransits <- merge(dailyTransitsCount, dailyTransitsPlanetYCount, by = "Date")
  dailyAspects <- merge(dailyAspects, dailyTransits, by = "Date")
