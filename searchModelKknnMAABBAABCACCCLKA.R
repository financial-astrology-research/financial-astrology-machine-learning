...
#            16) Add MO, ME max/min norm speed features to model data.
...
modelId <- "ensamble-kknn-MAABBAABCACCCLKA"
zdiffPercentCut <- 3
maPriceFsPeriod <- 2
maPriceSlPeriod <- 3
...
dailyPlanetsSpeed <- dailyPlanetsSpeedTablePrepare(hourlyPlanets = hourlyPlanets, pSelect = pSpeedSelect)
...
searchModel <- function(symbol) {
  ...
  prepareDailyAspects <- function() {
    ...
    dailyAspects <- merge(dailyAspects, dailyPlanetsSpeed, by = "Date")
  ...
  modelTrain <- function() {
    cat(
      "Using", modelId, "PX:", pxSelectAll, "- PY:", pySelectAll,
      "- PDEC:", pDecSelect, "- PSP:", pSpeedSelect,
      "- AORB:", orbLimit, "- SORB:", sepOrbLimit, "\n"
    )
