# Title     : Daily planets angular (aggregated) and longitude harmonics.
...
# CONCLUSION:
# 1) Combined angular harmonics may provide angles compression to avoid overfit and still provides
#    predictive power but needs to aggregate the relevant harmonics for each asset.
...
aggregateAngularHarmonics <- function(x) {
  sum(x)
}
...
selectColumns <- columnNames[grep(planetPairsPattern, columnNames)]
dailyPlanets <- hourlyPlanets[, lapply(.SD, mean), .SDcols = selectColumns, by = 'Date']
...
featureColumns <- c('Date', paste0(pySelect, 'W'))
dailyAspects <- dailyPlanets[, ..featureColumns]
