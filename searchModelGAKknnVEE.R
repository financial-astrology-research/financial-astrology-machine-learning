# Title     : Daily generalized aspects energy with planets weight and speed decay data KNN regression model
...
#             1) Planets ME, VE, SU, MA, VS, CE, JU, SA, NN planets applying to all planets, asteroids and NN.
...
searchModel <- function(symbol) {
  ...
  prepareDailyAspects <- function(pxSelect, pySelect, planetWeight, orbLimit) {
    dailyAspects <- dailyAspectsGeneralizedEnergySum(
      ...
      energyFunction = partial(dailyAspectsAddEnergyWeightWithDecay3, speedDecay = 0.59, planetWeight = planetWeight)
    )
