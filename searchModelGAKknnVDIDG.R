...
#             1) Planets ME, VE, SU, MA, VS, JU, SA, NN fast planets applying to all planets, asteroids and NN.
...
#             9) GA feature selection popSize = 100 and iter = 100.
...
#            13) Minor aspects set: 120/135/150/180, Medium: 30/45/60 Major: 0/90
#            14) Ignore separative aspects.
...
dailyAspectsRows[aspect %in% c(120, 135, 150, 180) & orb > 2, filter := T]
dailyAspectsRows[aspect %in% c(30, 45, 60) & orb > 3, filter := T]
dailyAspectsRows[aspect %in% c(0, 90) & orb > 4, filter := T]
dailyAspectsRows[type == 'S', filter := T]
