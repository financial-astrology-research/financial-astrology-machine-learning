...
#            10) GA KKNN K search in range 5-10.
...
modelId <- "ensamble-gakknn-MAABBAABCAAB"
zdiffPercentCut <- 3
maPriceFsPeriod <- 2
maPriceSlPeriod <- 3
...
searchModel <- function(symbol) {
  ...
  prepareModelData <- function(params) {
    c(pxSelect, pySelect, orbLimit, kMax) %<-% params
  ...
  modelTrain <- function(params) {
    c(pxSelect, pySelect, orbLimit, kMax) %<-% params
    cat("Using PX:", pxSelect, "- PY:", pySelect, "- AORB: ", orbLimit, "- kMax: ", kMax, "\n")
  ...
  gar <- ga(
    ...
    lower = c(2, 5, rep(0, gaParamsNum-1)),
    upper = c(3, 10, rep(1, gaParamsNum-1)),
