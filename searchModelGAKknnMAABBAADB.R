...
#             1) Planets MO, ME, VE, SU fast planets applying to all planets except SU.
...
#            12) GA aspect specific orb search in range 0-4.
...
modelId <- "ensamble-gakknn-MAABBAADB"
zdiffPercentCut <- 3
maPriceFsPeriod <- 2
maPriceSlPeriod <- 3
...
gaParamsNum <- length(aspectsSelectAll) + length(pySelectAll)
idCols <- c('Date', 'Hour')
...
searchModel <- function(symbol) {
  ...
  prepareDailyAspects <- function(params) {
    c(pxSelect, pySelect, aspectsOrb) %<-% params
    dailyAspectsRowsIter <- selectAspectsWithCustomOrb(dailyAspectsRows, aspectsOrb)
    dailyAspects <- prepareDailyAspectsCountPlanetYActivationCount(
      dailyAspectsRowsIter, orbLimit, pxSelect, pySelect, aspectsSelectAll
    )
  ...
  prepareModelData <- function(params) {
    dailyAspects <- prepareDailyAspects(params)
  ...
  modelTrain <- function(params) {
    c(pxSelect, pySelect, aspectsOrb) %<-% params
    cat("Using PX:", pxSelect, "- PY:", pySelect, "- AORB: ", aspectsOrb, "\n")
  ...
  parseSolutionParameters <- function(solution) {
    ...
    aspectsOrb <- solution[1:10]
    pySelect <- pySelectAll[solution[11:gaParamsNum] == 1]

    return(list(
      pxSelect = pxSelect,
      pySelect = pySelect,
      aspectsOrb = aspectsOrb
    ))
  }
  ...
  gar <- ga(
    ...
    lower = c(rep(0, length(aspectsSelectAll)), rep(0, length(pySelectAll))),
    upper = c(rep(4, length(aspectsSelectAll)), rep(1, length(pySelectAll))),
