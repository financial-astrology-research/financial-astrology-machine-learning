...
#            12) GA search orb limit 2-5 degrees.
#            13) GA search sep orb limit 0-3 degrees.
#            14) Z diff percent cut to 3.
#            15) Remove pxSelect from GA search.
...
modelId <- "ensamble-gakknn-MAABBAABAB"
zdiffPercentCut <- 3
maPriceFsPeriod <- 2
maPriceSlPeriod <- 3
...
dailyAspectsRows <- dailyHourlyAspectsTablePrepare(hourlyPlanets, idCols, 5)
...
searchModel <- function(symbol) {
  ...
  prepareDailyAspects <- function(params) {
    c(pxSelect, pySelect, orbLimit, sepOrbLimit) %<-% params
  ...
  modelTrain <- function(params) {
    c(pxSelect, pySelect, orbLimit, sepOrbLimit) %<-% params
    cat("Using PX:", pxSelect, "- PY:", pySelect, "- AORB: ", orbLimit, "- SORB: ", sepOrbLimit, "\n")
  ...
  gar <- ga(
    ...
    lower = c(2, 0, rep(0, gaParamsNum-1)),
    upper = c(5, 3, rep(1, gaParamsNum-1)),
