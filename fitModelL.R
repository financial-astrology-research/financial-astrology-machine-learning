...
modelSearch <- glmulti(
  ...
  method = "g", plotty = F,
  popsize = 200, mutrate = 0.01, sexrate = 0.1, imm = 0.1,
)
...
cor(aspectViewTest$diffPercent, aspectViewTest$diffPredict) %>% print()
plot(aspectViewTest$diffPercent, aspectViewTest$diffPredict)
with(aspectViewTest, mean((diffPercent - diffPredict) ^ 2)) %>% sqrt()

# Validate with reserved data.
securityDataTest <- mainOpenSecurity(symbol, 14, 28, "%Y-%m-%d", "2020-08-01")
