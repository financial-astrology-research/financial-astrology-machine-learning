...
#             1) Planets MO, ME, VE, SU, MA, JU, SA fast planets applying to all slow planets, asteroids and NN.
...
#            12) Include pxSelect in GA solution search.
...
searchModel <- function(symbol) {
  ...
  parseSolutionParameters <- function(solution) {
    pxSelect <- pxSelectAll[solution[1:7] == 1]
    pySelect <- pySelectAll[solution[8:nBits] == 1]
