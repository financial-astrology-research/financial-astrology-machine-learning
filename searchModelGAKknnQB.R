...
# TODO: Continue train this model and create variation.
modelId <- "ensamble-gakknn-QB"
zscoreCut <- 2
maPriceFsPeriod <- 2
maPriceSlPeriod <- 3
...
searchModel <- function(symbol) {
  ...
  prepareDailyAspects <- function(pxSelect, pySelect, aspectSelect) {
    ...
    dailyPlanetXActivationCount <- dailyPlanetXActivationCount(
      dailyAspects = dailyAspectsRows,
      orbLimit = orbLimit,
      pxSelect = pxSelect,
      pySelect = pySelect,
    ...
    if (is.null(dailyPlanetXActivationCount)) {
      return(NULL)
    }
    ...
    dailyAspects <- merge(dailyAspectsGeneralizedCount, dailyPlanetXActivationCount, date = "Date")
    dailyAspects <- merge(dailyAspects, dailyPlanetYActivationCount, date = "Date")
