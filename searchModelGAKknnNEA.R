...
#             6) Fit 5 weak learners for zdifflogOxHL change.
...
#            11) Perform zdifflogOxHL zscore outliers drop.
...
searchModel <- function(symbol) {
  ...
  hist(securityData$zdifflogOxHL)
  ...
  prepareModelData <- function(params) {
    ...
    aspectView <- merge(
      securityData[, c('Date', 'zdifflogOxHL', 'HxLEff')],
      dailyAspects, by = "Date"
    )

    trainIndex <- createDataPartition(aspectView$zdifflogOxHL, p = 0.80, list = FALSE)
    aspectViewTrain <- aspectView[trainIndex,]
    aspectViewValidate <- aspectView[-trainIndex,]
    useFeatures <- names(dailyAspects)[-1]
    selectCols <- c('zdifflogOxHL', useFeatures)
  ...
  findRelevantFeatures <- function(solution) {
    ...
    for (i in 1:5) {
      ...
      validateMAE <- mae(modelData$validate$zdifflogOxHL, validateDiffPercentPred)
      validateRMSE <- rmse(modelData$validate$zdifflogOxHL, validateDiffPercentPred)
      validateR2 <- cor(modelData$validate$zdifflogOxHL, validateDiffPercentPred)^2
