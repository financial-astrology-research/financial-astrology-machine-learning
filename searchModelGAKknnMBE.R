# Title     : Daily planet generalized aspects + comb planets activation KNN regression model
...
#             2) CV folds to 5 with 2 repeats for weak learners, folds to 5 with 10 repeats for ensamble.
...
searchModel <- function(symbol) {
  ...
  prepareDailyAspects <- function(pxSelect, pySelect, aspectSelect) {
    ...
    if (is.null(dailyAspectsPlanetsActivation)) {
      return(NULL)
    }

    dailyAspects <- merge(dailyAspectsGeneralizedCount, dailyAspectsPlanetsActivation, date = "Date")
