# Title     : Daily generalized aspects energy with aspects * px * py weight and speed decay data KNN regression model
...
#             1) Planets MO, ME, VE, SU, MA, CE, VS, JU, SA, NN planets applying to all planets, asteroids and NN.
...
searchModel <- function(symbol) {
  ...
  prepareDailyAspects <- function(pxSelect, pySelect, planetWeight, aspectWeight, orbLimit) {
    dailyAspects <- dailyAspectsGeneralizedEnergySum(
      ...
      energyFunction = partial(
        dailyAspectsAddEnergyWeightWithDecay5,
        speedDecay = 0.59,
