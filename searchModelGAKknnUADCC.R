# Title     : Daily generalized aspects energy + PY activation energy (orb decay) data KNN regression model
...
#             1) Planets ME, VE, SU, MA, VS, CE, JU, SA fast planets applying to all planets and VS.
...
#            12) GA search separative aspects cut orb.
#            13) GA search aspects orb limit.
...
pySelectAll <- planetsBaseCols
hourlyPlanets <<- openHourlyPlanets('planets_12', clear = F)
dailyAspectsRows <- dailyHourlyAspectsTablePrepare(hourlyPlanets, idCols, 6)
...
searchModel <- function(symbol) {
  ...
  prepareDailyAspects <- function(pxSelect, pySelect, aspectSelect, orbLimit, sepAspectsOrb) {
    dailyAspectsRowsIter <- selectSeparativeAspectsWithCustomOrb(dailyAspectsRows, sepAspectsOrb)
    dailyAspectsGeneralizedEnergy <- dailyAspectsGeneralizedEnergySum(
      dailyAspects = dailyAspectsRowsIter,
      orbLimit = orbLimit,
      pxSelect = pxSelect,
      pySelect = pySelect,
    ...
    if (is.null(dailyAspectsGeneralizedEnergy)) {
      return(NULL)
    }

    dailyPlanetYActivationEnergy <- dailyPlanetYActivationEnergy(
      dailyAspects = dailyAspectsRowsIter,
      orbLimit = orbLimit,
      pxSelect = pxSelect,
      pySelect = pySelect,
    ...
    dailyAspects <- merge(dailyAspectsGeneralizedEnergy, dailyPlanetYActivationEnergy, date = "Date")

    return(dailyAspects)
  }

  prepareModelData <- function(params) {
    c(pxSelect, pySelect, aspectSelect, orbLimit, sepAspectsOrb) %<-% params
    dailyAspects <- prepareDailyAspects(pxSelect, pySelect, aspectSelect, orbLimit, sepAspectsOrb)
  ...
  modelTrain <- function(params) {
    c(pxSelect, pySelect, aspectSelect, orbLimit, sepAspectsOrb) %<-% params
    cat("Using PX:", pxSelect, "- PY:", pySelect, "- ASP:", aspectSelect, "- ORB: ", orbLimit, "- SORB:", sepAspectsOrb, "\n")
  ...
  parseSolutionParameters <- function(solution) {
    ...
    sepAspectsOrb <- solution[1]
    orbLimit <- solution[2]
    pySelect <- pySelectAll[solution[3:gaParamsNum] == 1]
  ...
  gar <- ga(
    ...
    lower = c(0, 1, rep(0, gaParamsNum-2)),
    upper = c(4, 5, rep(1, gaParamsNum-2)),
