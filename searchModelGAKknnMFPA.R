...
#             1) Planets MO, ME, VE, SU fast planets applying to all planets, VS, CE, NN and SN.
...
#             5) GA feature detection that fit to minimize RMSE train data.
...
searchModel <- function(symbol) {
  ...
  findRelevantFeatures <- function(solution) {
    ...
    for (i in 1:5) {
      ...
      allR2 <- c(allR2, validateR2)
      allRMSE <- c(allRMSE, validateRMSE)
    }

    fitEffectR2 <- mean(allR2) / (1 + sd(allR2))
    fitEffectRMSE <- mean(allRMSE) / (1 + sd(allRMSE))
    cat("Final effects RMSE:", fitEffectRMSE, "R2:", fitEffectR2, "\n\n")

    return(-fitEffectRMSE)
  }
