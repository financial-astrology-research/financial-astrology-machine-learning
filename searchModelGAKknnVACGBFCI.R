...
#            14) Minor aspects set: 120/135/150/180, Medium: 30/45 Major: 0/60/90
...
modelId <- "ensamble-gakknn-VACGBFCI"
zscoreCut <- 2
maPriceFsPeriod <- 2
maPriceSlPeriod <- 3
...
dailyAspectsRows[aspect %in% c(135, 150, 180, 120) & orb > 2, filter := T]
dailyAspectsRows[aspect %in% c(30, 45) & orb > 3, filter := T]
dailyAspectsRows[aspect %in% c(0, 60, 90) & orb > 4, filter := T]
