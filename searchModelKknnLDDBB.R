...
#             1) Planets MO, ME, VE, SU fast planets applying to all planets and NN.
...
#             11) Add planets zodiacal qualities count features.

# CONCLUSIONS:
# 1) When added zodiac sign planets count features the model overfit.
# 2) When added zodiac element planets count features the model overfit.
# 3) When added zodiac quality planets count features the model don't improve but could be significant.
# 4) When added zodiac polarity planets count features the model don't improve.
# 5) When added zodiac decan planets count features the model overfit.
# 6) When added zodiac arab mansions planets count features the model overfit.
# 7) When added zodiac vedic mansions planets count features the model overfit.
# 8) When added planet speed phases count features the model overfit.

# TODO: Use as root for new models generation.
...
dailyPlanetsPositionsLong <- fread("./dplanets/daily_planets_positions_long.csv")
dailyPlanetsPositionsLong <- dailyPlanetsPositionsLong[pID %in% pxSelect]
dailyPlanetsPositionsLong[, pID := paste0(pID, "Z")]
dailyPlanetsPositions <- data.table::dcast(
  dailyPlanetsPositionsLong,
  Date ~ QualityID,
  value.var = c('pID'),
  fun.aggregate = length
)
dailyPlanetsPositions[, Date := as.Date(Date)]
...
dailyAspects <- merge(dailyAspects, dailyPlanetsPositions, by = "Date")
