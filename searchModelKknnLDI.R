...
dailyAspectsCount <- dailyAspectsGeneralizedCountLDI(
  orbLimit = orbLimit,
  pxSelect = pxSelect,
  pySelect = pySelect
)

dailyAspectsPlanetYCount <- dailyPlanetYActivationCountLDI(
  orbLimit = orbLimit,
  pxSelect = pxSelect,
  pySelect = pySelect
)
...
fwrite(dailyAspects[, ..exportCols], paste("./predictions/", symbol, "-predict-kknnLDI-ensamble", ".csv", sep = ""))
