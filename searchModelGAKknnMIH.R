# Title     : Daily planetY aspects count split by polarity data KNN regression model
...
#             1) Planets MO, ME, VE fast planets applying to all slow planets, asteroids and NN.
...
searchModel <- function(symbol) {
  ...
  prepareDailyAspects <- function(pxSelect, pySelect, aspectSelect, aspectsPolarity) {
    dailyPlanetYActivationPolarityCount <- dailyPlanetYActivationPolarityCount(
      dailyAspects = dailyAspectsRows,
      orbLimit = orbLimit,
      pxSelect = pxSelect,
      pySelect = pySelect,
      aspectSelect = aspectSelect,
      polarityFunction = partial(dailyAspectsAddPolarity, aspectsPolarity = aspectsPolarity)
    )

    if (is.null(dailyPlanetYActivationPolarityCount)) {
      return(NULL)
    }

    return(dailyPlanetYActivationPolarityCount)
  }

  prepareModelData <- function(params) {
    c(pxSelect, pySelect, aspectSelect, aspectsPolarity) %<-% params
    dailyAspects <- prepareDailyAspects(pxSelect, pySelect, aspectSelect, aspectsPolarity)
  ...
  modelTrain <- function(params) {
    c(pxSelect, pySelect, aspectSelect, aspectsPolarity) %<-% params
    cat("Using PX:", pxSelect, "- PY:", pySelect, "- ASP:", aspectsPolarity, "\n")
  ...
  parseSolutionParameters <- function(solution) {
    ...
    aspectsPolarity <- solution[14:gaParamsNum]

    return(list(
      pxSelect = pxSelect,
      pySelect = pySelect,
      aspectSelect = aspectSelect,
      aspectsPolarity = aspectsPolarity
    ))
  }
