...
#             8) Feature selection based on higher correlation with diff percent change.
...
modelTrain <- function(featureCorrelationCut, modelId) {
  ...
  aspectView <- merge(
    securityData[, c('Date', 'Actbin', 'diffPercent')],
    dailyAspects, by = "Date"
  )

  varCorrelations <- aspectView[, -c(1, 2)] %>%
    cor() %>%
    round(digits = 2)
  finalCorrelations <- sort(varCorrelations[, 1])
  cat("Features diffPercent correlation: \n")
  print(finalCorrelations)
  cat("\n\n")

  if (featureCorrelationCut > 0) {
    relevantFeatures <- names(
      finalCorrelations[finalCorrelations >= featureCorrelationCut]
    )
  }
  else {
    relevantFeatures <- names(
  ...
  selectCols <- unique(c("diffPercent", relevantFeatures))
...
fitModel1 <- modelTrain(-0.03, "1")
fitModel2 <- modelTrain(-0.04, "2")
fitModel3 <- modelTrain(-0.05, "3")
fitModel4 <- modelTrain(0.03, "4")
...
fwrite(dailyAspects[, ..exportCols], paste("./predictions/", symbol, "-predict-kknnLDR-ensamble", ".csv", sep = ""))
