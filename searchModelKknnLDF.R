...
#             1) ME + VE + SU fast planets applying to all slow planets except NN.
...
#             8) Orb to 4 degrees.
#             9) 3 weak learners using ME+VE.
#            10) 2 weak learners using ME+VE+SU.
...
dailyAspectsCount1 <- dailyAspectsGeneralizedCount(
  orbLimit = orbLimit,
  pxSelect = pxSelect1,
  pySelect = pySelect
)

dailyAspectsPlanetYCount1 <- dailyPlanetYActivationCount(
  orbLimit = orbLimit,
  pxSelect = pxSelect1,
  pySelect = pySelect
)

dailyAspectsCount2 <- dailyAspectsGeneralizedCount(
  orbLimit = orbLimit,
  pxSelect = pxSelect2,
  pySelect = pySelect
)

dailyAspectsPlanetYCount2 <- dailyPlanetYActivationCount(
  orbLimit = orbLimit,
  pxSelect = pxSelect2,
  pySelect = pySelect
)

dailyAspects1 <- merge(dailyAspectsCount1, dailyAspectsPlanetYCount1, by = "Date")
dailyAspects2 <- merge(dailyAspectsCount2, dailyAspectsPlanetYCount2, by = "Date")
dailyAspects <- merge(dailyAspects1, dailyAspects2,  by = "Date")
...
useFeatures1 <- allFeatures[grep('\\.x$', allFeatures)]
useFeatures2 <- allFeatures[grep('\\.y$', allFeatures)]
fitModel1 <- modelTrain(
  "kknn", useFeatures1, maPriceFsPeriod, maPriceSlPeriod, "1"
)

fitModel2 <- modelTrain(
  "kknn", useFeatures1, maPriceFsPeriod, maPriceSlPeriod, "2"
)

fitModel3 <- modelTrain(
  "kknn", useFeatures1, maPriceFsPeriod, maPriceSlPeriod, "3"
)

fitModel4 <- modelTrain(
  "kknn", useFeatures2, maPriceFsPeriod, maPriceSlPeriod, "4"
)

fitModel5 <- modelTrain(
  "kknn", useFeatures2, maPriceFsPeriod, maPriceSlPeriod, "5"
)
...
fwrite(dailyAspects[, ..exportCols], paste("./predictions/", symbol, "-predict-kknnLDF-ensamble", ".csv", sep = ""))
