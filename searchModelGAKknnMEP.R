...
#            13) Always include VE as fast planet (emitter).
...
searchModel <- function(symbol) {
  ...
  parseSolutionParameters <- function(solution) {
    pxSelect <- c("VE", pxSelectAll[solution[1:3] == 1])
    pySelect <- pySelectAll
    aspectSelect <- aspectSelectAll
    aspectsEnergy <- solution[4:gaParamsNum]
